...
# Endpoint for generating songs based on user preferences
@app.post("/generate_song/")
async def generate_song(song_request: SongRequest):
    sentiment_preference = song_request.sentiment_preference
    genre = song_request.genre
    topics = song_request.topics
    
    # Use your existing function `generate_songs` to fetch the articles, generate summaries, and create the song
    results = generate_songs(topics, genre, sentiment_preference)
    
    return {"status": "success", "data": results}

# Use your existing logic for generating songs and lyrics here
...
def generate_avatar_from_lyrics(lyrics, topic, genre):
    ...
    The avatar should visually represent the essence and mood of the song and its topic. Make sure the visual style fits the {genre} genre. Please return only the prompt to be used for the image-generating API, ensuring it's concise but descriptive enough to capture the spirit of the song.
    """
...
async def test_generate():
    ...
    return await generate_song(sample_request)